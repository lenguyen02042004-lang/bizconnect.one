...
test.describe("Personal Account E2E Flow", () => {
  ...
  test("User can register, setup profile, use inbox, and view pricing", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText(/Personal Card|Danh thiếp cá nhân/i);

    // 3. Messages / Inbox
    await page
      .locator("a", { hasText: /Hộp thư|Inbox/i })
      .first()
      .click();
    ...
    await page
      .locator("a", { hasText: /Danh bạ|Following|Contacts/i })
      .first()
      .click();
    ...
    // 5. Upgrades / Pricing
    await page
      .locator("a", { hasText: /Bảng giá|Pricing/i })
      .first()
      .click();
