...
export function BulkImportSection({
  ...
  importFn: ReturnType<typeof useServerFn<typeof bulkImportBusinesses>>;
}) {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<Record<string, string>[]>([]);
  const [result, setResult] = useState<Awaited<ReturnType<typeof bulkImportBusinesses>> | null>(
    null,
  );
  ...
  function exportCredentials() {
    ...
    const body = result.credentials
      .map((c) =>
        [c.row, c.slug, c.email, c.password, c.created ? "yes" : "no"]
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "business-import-credentials.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  return (
    <section className="rounded-3xl bg-card border border-border p-6">
      <h2 className="font-display text-xl font-semibold mb-2">
        Nhập hàng loạt doanh nghiệp + tài khoản (CSV)
      </h2>
      <p className="text-sm text-muted-foreground mb-2">
        Mỗi dòng CSV tạo <strong>1 doanh nghiệp</strong> và (nếu bật){" "}
        <strong>tài khoản chủ sở hữu</strong> tương ứng. Cột bắt buộc: <code>owner_email</code>,{" "}
        <code>name</code>, <code>slug</code>. Tùy chọn: <code>owner_password</code> (nếu để trống,
        hệ thống sinh mật khẩu ngẫu nhiên mạnh riêng cho từng tài khoản),{" "}
        <code>owner_display_name</code>. Hãy tải CSV mật khẩu ngay sau khi import — mật khẩu chỉ
        hiển thị một lần.
      </p>
        ...
        <div className="p-4 rounded-xl bg-muted space-y-3">
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">Doanh nghiệp: {result.ok}</span>
            </div>
            <div className="flex items-center gap-2">
              <Users className="text-primary w-5 h-5" />
              <span className="font-semibold">Tài khoản mới: {result.created_users}</span>
            </div>
            {result.failed.length > 0 && (
              <div className="flex items-center gap-2">
                <AlertCircle className="text-destructive w-5 h-5" />
                <span className="font-semibold">Lỗi: {result.failed.length}</span>
              </div>
            )}
          </div>
