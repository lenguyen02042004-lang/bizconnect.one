...
export const getBusinessBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    let query = supabase.from("businesses").select("*, industries(name, slug), countries(name)");
    ...
    return {
      business: {
        ...
        claimed_at: (biz as any).claimed_at ?? null,
        premium_until: (biz as any).premium_until ?? null,
...
export const getGlobalLists = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data: activeBizes } = await supabase.rpc("get_public_country_codes");
