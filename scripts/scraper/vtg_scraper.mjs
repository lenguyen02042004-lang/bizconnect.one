...
import { supabase, getCountryCode, slugify, setupSystemAccount, setupIndustry, randomDelay } from "./utils.mjs";

(async () => {
  try {
    console.log("--- STARTING VTG SCRAPER ---");
    const ownerId = await setupSystemAccount();
    const industryId = await setupIndustry("Textile & Garment");
    ...
    while (keepGoing) {
      ...
      for (let retries = 0; retries < 3; retries++) {
        try {
          await page.goto(`https://vtg.chanchao.com.tw/VisitorExhibitor?page=${pageNum}`, {
      ...
      if (companies.length === 0) {
          console.log(`No companies found on page ${pageNum}. Assuming end of list.`);
          keepGoing = false;
          break;
      }
      ...
      for (let i = 0; i < companies.length; i++) {
        ...
        if (comp.detailUrl) {
          console.log(`[${i + 1}/${companies.length}] Sleeping to prevent block...`);
          await randomDelay(10000, 20000); // Wait between 10 to 20 seconds
          console.log(`Getting details for ${comp.name}...`);
